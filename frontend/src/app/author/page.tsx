...
export default function AuthorDashboard() {
  ...
  return (
    <>
      ...
      {data && data.length === 0 && <p className="mt-4 text-ink/60">You have not submitted a manuscript yet.</p>}
      <ul className="mt-4 space-y-3">
        {data?.map((manuscript) => (
          <li key={manuscript.tracking_code}>
            <Link
              href={`/author/${manuscript.tracking_code}`}
              className="flex items-center justify-between rounded-[3px] border border-rule bg-white/70 p-4 hover:border-teal/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber"
            >
              <div>
